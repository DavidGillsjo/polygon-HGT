...
class DatasetCatalog(object):
    ...
    DATASETS.update({
        'structured3D_perspective_planes_{}'.format(d): {
            'img_dir': 'Structured3D_simple_planes_with_parameters/images',
            'ann_file': 'Structured3D_simple_planes_with_parameters/{}.json'.format(d)}
        for d in ('train', 'train_mini', 'test', 'test_mini', 'val', 'val_mini')
    })

    @staticmethod
    def get(name, tmp_dir = None):
        assert name in DatasetCatalog.DATASETS
        data_dir = tmp_dir if tmp_dir else DatasetCatalog.DATA_DIR
