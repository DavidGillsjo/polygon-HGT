...
class Checkpointer(object):
    ...
    def load(self, f=None, f_plane=None, use_latest=True, transfer = False):
        ...
        if f_plane:
            plane_checkpoint = self._load_file(f_plane)
            checkpoint = self._merge_models(checkpoint, plane_checkpoint)
        ...
        if "wandb_id" in checkpoint and not transfer:
            self.logger.info("Loading wandb_id from {}".format(f))
            self.wandb_id = checkpoint.pop("wandb_id")

        # return any further checkpoint data
        return checkpoint

    # Assumes we convert WireframeDetector and WireframeGNNClassifier to WireframeDetector with WireframeGNNHead
    def _merge_models(self, checkpoint, plane_checkpoint):
        new_model = {}
        model = checkpoint['model']
        for k,v in model.items():
            if not k.startswith('plane_classifier'):
                new_model[k] = v
        plane_model = plane_checkpoint['model']
        for k,v in plane_model.items():
            if k.startswith('plane_classifier'):
                new_model[k] = v
...
class DetectronCheckpointer(Checkpointer):
    def __init__(
        ...
        super(DetectronCheckpointer, self).__init__(
            model, optimizer, scheduler, save_dir, save_to_disk, logger,wandb_id
        )
        self.cfg = cfg.clone()
