    ...
    def __call__(self, image, ann):
        ...
        for plane_type in ['planes', 'planes_negative']:
            for plane in ann[plane_type]:
                plane['centroid'][0] = np.clip(plane['centroid'][0]*sx, 0, self.ann_width-1e-4)
                plane['centroid'][1] = np.clip(plane['centroid'][1]*sy, 0, self.ann_height-1e-4)
...
class ToTensor(object):
    def __call__(self, image, anns=None):
        ...
        for key,val in anns.items():
            ...
            # Handles the list of plane dictionaries
            if isinstance(val, list) and isinstance(val[0], dict):
                anns[key] = [self(image, v)[1] for v in val]
...
class NormalizePlaneParameters(object):
    def __init__(self, mean, std):
        self.mean = torch.tensor(mean)
        self.std  = torch.tensor(std)
    def __call__(self, image, anns = None):
        for p in anns['planes']:
            p['parameters'] = (p['parameters'] - self.mean)/self.std
        return image, anns
